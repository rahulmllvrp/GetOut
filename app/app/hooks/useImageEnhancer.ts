...
export function useImageEnhancer(captureCanvas: () => string) {
  ...
  const showHiddenPov = async (cacheKey: string, description: string) => {
    ...
    const memoryCacheKey = `${cacheKey}-hidden-image`;

    // L1 — in-memory cache
    if (imageCache.current.has(memoryCacheKey)) {
      setEnhancedImageUrl(imageCache.current.get(memoryCacheKey)!);
      requestAnimationFrame(() => setOverlayVisible(true));
      return;
    }

    // L2 — disk cache (pre-generated by prewarm)
    const diskUrl = `/enhanced-cache/pov-${cacheKey}.png`;
    const diskCheck = await fetch(diskUrl, { method: "HEAD" });
    if (diskCheck.ok) {
      imageCache.current.set(memoryCacheKey, diskUrl);
      setEnhancedImageUrl(diskUrl);
      requestAnimationFrame(() => setOverlayVisible(true));
      return;
    }

    // L3 — fallback: on-demand Gemini generation
    ...
    try {
      ...
      const { imageDataUrl } = await res.json();
      imageCache.current.set(memoryCacheKey, imageDataUrl);
      setEnhancedImageUrl(imageDataUrl);
      requestAnimationFrame(() => setOverlayVisible(true));
    } finally {
      isEnhancingRef.current = false;
